import { mergeProps, useSSRContext, ref, unref, onMounted, onUnmounted, withCtx, createVNode, toDisplayString, createTextVNode, openBlock, createBlock, withDirectives, isRef, Fragment, renderList, vModelSelect, createCommentVNode } from "vue";
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(AuthLayout, null, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<header class="p-2 text-xl font-bold flex items-center justify-between"${_scopeId}><h2 class=""${_scopeId}>${ssrInterpolate(unref(props).name)}</h2><p${_scopeId}>${ssrInterpolate(_ctx.trans("words.max_players"))} : ${ssrInterpolate(unref(props).max_players)}</p></header>`);
            if (unref(props).hostId === unref(page).auth.user.id) {
              _push2(`<form${_scopeId}><section class="my-1 px-2"${_scopeId}><label for="select" class="block text-sm font-medium text-gray-700"${_scopeId}>Story:</label><section class="mt-1 relative"${_scopeId}><select id="select" class="block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md"${_scopeId}><option value="" disabled${_scopeId}>Select an option</option><!--[-->`);
              ...
              _push2(`<!--]--></select></section></section></form>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`<section class="mb-2 bg-zinc-400 px-2 border border-gray-600 rounded-md shadow-lg"${_scopeId}><section class="flex gap-1 m-2 px-2 py-4 border rounded-lg overflow-x-auto"${_scopeId}><!--[-->`);
            ...
            if (unref(props).hostId === unref(page).auth.user.id) {
              ...
              if (unref(selectedStory)) {
                _push2(ssrRenderComponent(unref(Link), {
                  href: _ctx.route("lobby.start", [__props.lobbyId, unref(selectedStory)]),
            ...
            return [
              ...
              unref(props).hostId === unref(page).auth.user.id ? (openBlock(), createBlock("form", { key: 0 }, [
                createVNode("section", { class: "my-1 px-2" }, [
                  ...
                  createVNode("section", { class: "mt-1 relative" }, [
                    withDirectives(createVNode("select", {
                      id: "select",
                      "onUpdate:modelValue": ($event) => isRef(selectedStory) ? selectedStory.value = $event : selectedStory = $event,
                      ...
                      [vModelSelect, unref(selectedStory)]
                    ])
                  ])
                ])
              ...
              createVNode("section", { class: "flex items-center justify-center gap-2 my-2" }, [
                unref(props).hostId === unref(page).auth.user.id ? (openBlock(), createBlock(Fragment, { key: 0 }, [
                  ...
                  unref(selectedStory) ? (openBlock(), createBlock(unref(Link), {
                    key: 0,
                    href: _ctx.route("lobby.start", [__props.lobbyId, unref(selectedStory)]),
                    class: "btn btn-primary"
                  }, {
